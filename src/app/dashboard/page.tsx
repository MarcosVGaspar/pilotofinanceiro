...
export default async function DashboardPage() {
  ...
  const kpis = [
    ...
    { label: 'Rendas Extras',  value: fmt$(totalRendas),   color: 'var(--success)',  icon: '💰', sub: `${(rendas||[]).length} lançamentos`,    delay: 'delay-2' },
    { label: 'Despesas',       value: fmt$(totalDespesas), color: 'var(--danger)',   icon: '💳', sub: `${(despesas||[]).length} lançamentos`,  delay: 'delay-3' },
      ...
      {meta > 0 && (
        <div className="glass-card anim-up delay-2" style={{ padding: '20px 24px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            ...
            <p style={{ fontSize: '13px', fontWeight: 800, color: pct >= 100 ? 'var(--accent)' : 'var(--text-2)' }}>
              {pct.toFixed(0)}%
            </p>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
            <p style={{ fontSize: '12px', color: 'var(--text-3)' }}>
              {fmt$(totalCorridas + totalRendas)} arrecadado
            </p>
            <p style={{ fontSize: '12px', color: 'var(--text-3)' }}>
              {pct >= 100 ? '🎉 Meta atingida!' : `faltam ${fmt$(meta - totalCorridas - totalRendas)}`}
            </p>
          </div>
        </div>
      )}
